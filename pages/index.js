...
export default function Dashboard() {
  ...
  // Fetch metrics, feed posts, and item stock from backend
  useEffect(() => {
  ...
  // Submit a project update / devlog post
  const handleSubmitLog = async (e) => {
    ...
    if (res.ok) {
      ...
      setPoints(prev => prev + data.pointsGained);
      setNewLogText("");
      alert(`🎉 Devlog posted! You earned +${data.pointsGained} Stardust Points.`);
    }
  };

  // Process a virtual shop reward transaction
  const handlePurchase = async (item) => {
    ...
    if (res.ok) {
      ...
      alert(`🚀 Item unlocked! ${data.message}`);
    } else {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-4 sm:p-8">
      ...
      <header className="flex justify-between items-center border-b border-purple-900/40 pb-6 mb-8">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Stardance-Engine Node Integration</p>
        </div>
        <div className="bg-purple-950/40 border border-purple-500/30 px-5 py-2.5 rounded-2xl flex items-center gap-2 shadow-lg">
          ...
          <div>
            ...
            <p className="font-bold text-yellow-400 text-lg leading-tight">{points} ✨ Stardust</p>
          </div>
        </div>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Left Side: Submission Form & Devlog Feed */}
        <div className="lg:col-span-2 space-y-6">
          {/* Submit Log Card */}
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-md">
            <h2 className="text-lg font-semibold text-slate-200 mb-4 flex items-center gap-2">
              <PlusCircle size={18} className="text-purple-400" /> Log Project Progress
            </h2>
            <form onSubmit={handleSubmitLog} className="space-y-4">
              <textarea
                value={newLogText}
                onChange={(e) => setNewLogText(e.target.value)}
                placeholder="What teleporter or printing task did you work on today? Give details to earn stardust points..."
          ...
          <div className="space-y-4">
            <h2 className="text-lg font-semibold text-slate-300 px-1">Project Activity Logs</h2>
            <div className="space-y-3">
              {logs.length === 0 ? (
                <p className="text-sm text-slate-500 italic p-4">No logged project entries yet. Submit your first one above!</p>
              ) : (
                logs.map(log => (
                  <div key={log.id} className="bg-slate-900 border border-slate-800/80 p-4 rounded-xl flex justify-between items-center">
                    ...
                    <span className="bg-green-950/60 border border-green-500/20 text-green-400 text-xs font-bold px-2.5 py-1 rounded-lg flex items-center gap-1 shrink-0">
                      <CheckCircle2 size={12} /> +{log.pointsGained} Pts
                    </span>
                  </div>
                ))
              )}
        ...
        {/* Right Side: Virtual Currency Store */}
        <div className="space-y-6">
          <h2 className="text-lg font-semibold text-slate-300 px-1 flex items-center gap-2">
            <Box size={18} className="text-purple-400" /> Stardust Store
          </h2>
